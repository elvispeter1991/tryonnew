...
class EffectRenderer {
    constructor() {
        ...
        loader.load('TryOn.glb', (gltf) => {
            const scene = gltf.scene;
            const occluderMatt = new THREE.MeshBasicMaterial({colorWrite:false,  color: 0x00FFFF});
            scene.scale.set(1, 1, 1);
            scene.position.set(0,0,7);
            this.faceGroup.add(gltf.scene);
            var Glass = scene.getObjectByName("Plane004_Mustang_0");
            Glass.scale.set(0.95,0.95,0.95);
            Glass.position.set(0,-0.4,-0.6);
            Glass.rotation.set(0,0,0);
            var occluder = scene.getObjectByName("Model");
            occluder.position.set(0,0,-1);
           scene.traverse((o) => {
           
                if(o.isMesh){
                    if(o.material.name == "Material.002"){
                        o.material = occluderMatt;
                        o.renderOrder = Infinity;
